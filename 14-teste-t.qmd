...
Observa-se que existem `r nrow(dados)` alunos, sendo `r nrow(dados[which(dados$sexo=="Feminino"),])` mulheres e `r nrow(dados[which(dados$sexo=="Meninino"),])` homens. A variável `notas` é uma variável numérica que corresponde a a nota centesimal e `sexo` é uma variável categórica.
...
A saída informa que a média das notas das mulheres é `r round(resumo$media[1],1)`, bem acima das notas dos homens, mostrando uma diferença nos escores de `r round(resumo$media[1] - resumo$media[2],1)`. Parece que o desempenho das mulheres em Bioestatística é melhor do que o dos homens!
...
A saída retorna a estimativa da diferença média (`r teste$estimate`), as estimativas das médias dos grupos (arredondadas), a estatística do teste (`r teste$statistic`) o valor P (`r teste$p`), graus de liberdade (`r teste$df`)[^14-teste-t-3] e outras métricas.
...
O banco de dados é constituído por uma amostra de 15 escolares portadores de asma não controlada. Fizeram avaliação da sua função pulmonar no início do uso de um novo corticoide inalatório. Após 60 dias, repetiram a avaliação da função pulmonar[^14-teste-t-4].

[^14-teste-t-4]: O autor entende e recomenda que esta metodologia "ante e depois" deve ser evitada, pois traz consigo importante vieses como regressão à média, efeito placebo ou nocebo, viés temporal, viés de mensuração, etc. Ver @sec-trials . O uso aqui tem objetivo didático de mostrar a lógica estatística.
...
```{r}
#| label: fig-be
#| tbl-cap: "Gráfico de barra de erro comparando o grupo antes e depois"
#| echo: false
#| out.height: "70%"
#| out.width: "70%"
#| fig.align: "center"
...
```{r}
#| label: fig-bp
#| tbl-cap: "Boxplots comparando o grupo antes e depois"
#| echo: false
#| out.height: "70%"
#| out.width: "70%"
#| fig.align: "center"
...
```{r}
#| label: fig-lin
#| tbl-cap: "Gráfico de linha comparando o grupo antes e depois"
#| echo: false
#| out.height: "70%"
#| out.width: "70%"
#| fig.align: "center"
...
Além disso, um gráfico Q-Q (@fig-qq) pode ser usado para avaliar a normalidade, com a função `ggqqplot()` do pacote `ggpubr` @kassambara2022ggpubr que produz um gráfico QQ normal com uma linha de referência, acompanhada de area sombreada, correspondente ao IC95%
...
#| out.height: "70%"
#| out.width: "70%" 
#| fig.cap: "Gráfico de linha mostrando a diferença na resposta ao corticoide inalatório"
