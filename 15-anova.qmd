...
De um modo geral, o sofware estatístico calcula a **Soma dos Quadrados**(SQ) para cada uma das fontes de variação (sexo, tabagismo, interação e resíduos) e a partir delas, calcula os **Quadrados Médios** (QM)[^15-anova-15] e dividindo cada um pelos seus respectivos graus de liberdade, obtém-se a estatística F para cada efeito. O valor de F é comparado a uma distribuição F (veja a @sec-distF) para obter o valor *p* associado a cada hipótese, de acordo com o o valor de $\alpha$ escolhido previamente (em geral, 0,05) .

[^15-anova-15]: Que são as variâncias.
