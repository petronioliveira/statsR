...
#| out.height: "80%"
#| fig.align: 'center'
#| fig.cap: "Vinte lançamentos seguidos de uma moeda"
#| fig.pos: "H"
...
#| out.height: "80%"
#| fig.align: 'center'
#| fig.cap: "Proporção em 20 lançamentos de moeda"
#| fig.pos: "H"
...
#| out.height: "80%"
#| fig.align: 'center'
#| fig.cap: "Proporção à medida que N aumenta em lançamentos de moedas"
#| fig.pos: "H"
...
Observe que o valor de 1,60 m está muito próximo da média e isto é um indicativo de que essa variável tem uma distribuição praticamente simétrica. Sabendo a distância, em números de desvios padrão, que 1,60 m está da média, qual a probabilidade de encontrar, na maternidade do HGCS [^09-probabilidades-1], uma parturiente que tenha exatamente esta altura?\
Para responder a essa pergunta, será usada a função `pnorm()` (veja @sec-dnp) que utiliza o escore *Z*, a média e o desvio padrão para encontrar essa proporção que, multiplicada por 100, fornece a percentagem.
...
O *R* por padrão retorna números grandes como notação científica. O resultado dessa operação é um número tão grande que para escrevê-lo sem este tipo de notação, seriam necessários 127 dígitos decimais. O resultado não caberia em apenas uma linha. Ficaria assim, suprimindo a notação científica [^09-probabilidades-2]:

[^09-probabilidades-2]: Para remover a notação científica, usar a função options (scipen = 999) e, para desfazer essa ação, trocar o 999 por 0.
...
Para representar graficamente essa pequena probabilidade, será construída uma curva com essa pequena área sombreada, colorida em azul claro. Para isso, uma função própria, denominada `normal_intervalo_sombreado()` plotará a figura. Os argumentos dessa função são: a = limite inferior do intervalo; b = limite superior do intervalo; mean = média (padrão = 0); sd = desvio padrão (padrão = 1). Ela pode ser obtida [aqui](https://github.com/petronioliveira/Arquivos/blob/main/normal_intervalo_sombreado.R) para ser baixada em seu diretório de trabalho para uso posterior [^09-probabilidades-3]. Para carregar a função para uso usa-se a função nativa source() que serve para executar o código contido em um arquivo com a extensão .R.

[^09-probabilidades-3]: Veja @sec-funcpropria como se constrói uma função. A função apresentada aqui é mais complexa..

A @fig-prob1725 representa com clareza esta pequena probabilidade.
...
#| out.height: "70%"
#| fig.align: 'center'
#| fig.cap: "Probabilidade de encontrar mulheres com mais de 1,725m"
#| warning: false
...
ggplot(df1, aes(x = valor)) +
  ...
  geom_vline(xintercept = -1, color = "red", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = 1, color = "red", linetype = "dashed", size = 0.5) +
...
A variável `dados$altura`, mostrada na @sec-aplicafreq, tem uma média (`r round(mean(dados$altura), 3)`, mediana de (`r round(median(dados$altura), 3)`) e um coeficiente de variação (CV) muito pequeno de `r round(sd(dados$altura)/mean(dados$altura), 3)*100`%. Isso caracteriza uma variável com uma distribuição praticamente simétrica. Usando esses dados ($X\sim N(\mu=1,598,\sigma=0,065)$), pode-se calcular probabilidades, dadas pela área sob a curva.
...
#| out.height: "70%"
#| fig.align: 'center'
#| fig.cap: "Probabilidade de encontrar mulheres entre 1,47 e 1,725m"
#| warning: false
...
text(1.72, 6, cex = 1.3, bquote("N"~(mu==.(1.60)~","~sigma == .(0.065))))
axis(1, at = c(1.47,1.73), col.ticks = c("red" , "red"), 
     col.axis = "red")
```

A probabilidade de alturas entre 1,47m e 1,73m é igual a 95,4%, como mostrado na @fig-normal954.
...
1.  Qual a probabilidade de uma cirurgia de reconstrução do LCA requerer um tempo menor do que 100 minutos?

2.  Se uma cirurgia demorar 160 minutos, o que se conclui em relação a essa informação?
:::
...
normal_area(media = 129, dp = 14, 
            linf = 75, lsup = 100, 
            cor = "lightblue", lwd = 2 )
text(92, 0.0028, "1,92%", cex = 0.6)
text(158, 0.027, cex = 1.3, bquote("N"~(mu==.(129)~","~sigma == .(14))))
...
#| out.height: "80%"
#| fig.align: 'center'
#| fig.cap: "Probabilidade do tempo de cirurgia de LCA maior ou igual a 160 minutos"
#| warning: false
#| message: false
...
text(163, 0.027, cex = 1.3, bquote("N"~(mu==.(129)~","~sigma == .(14))))
...
1.  Qual a probabilidade de uma mulher com 150 cm pertencer a população 1?

2.  Qual a probabilidade de uma mulher com 150 cm pertencer a população 2?
:::
...
#| out.height: "80%"
#| fig.align: 'center'
#| fig.cap: "Probabilidade de uma mulher com 150 cmm pertencer a uma população de média igual a 160 cm"
#| warning: false
#| message: false
...
Na população 1, 6.5% das mulheres tem altura $\le 1,50$ m (@fig-normal649). Em outras palavras, existe pouca probabilidade dessa mulher pertencer à população 1.
...
#| out.height: "80%"
#| fig.align: 'center'
#| fig.cap: "Probabilidade de uma mulher com 150 cmm pertencer a uma população de média igual a 140 cm"
#| warning: false
#| message: false  
